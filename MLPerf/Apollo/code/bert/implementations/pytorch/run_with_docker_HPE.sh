...
echo "MLPERF_MODEL_CONSTANT is ${MLPERF_MODEL_CONSTANT}"
...
# end removed
docker run --privileged --rm --init --detach --gpus='"'device=${NV_GPU}'"' \
...
for _experiment_index in $(seq 1 "${NEXP}"); do
    (
        ...
        # Print system info
        docker exec -i "${CONT_NAME}" python -c "
...
log_event(key=constants.CACHE_CLEAR, value=True)"
        ...
        # Run experiment
        docker exec -i "${_config_env[@]}" "${CONT_NAME}" \
