...
echo "[run_and_time.sh] Hyper-parameters: seed=$SEED batch-size=$BATCHSIZE num-sample=$MAX_SAMPLES_TERMINATION target-acc=$TARGET_MLM_ACCURACY"

start=$(date +%s)
start_fmt=$(date +%Y-%m-%d\ %r)
echo "STARTING run_pretraining.py AT $start_fmt"

eval "${LOGGER} ${BERT_CMD} --seed=${SEED}"

start=$(date +%s)
start_fmt=$(date +%Y-%m-%d\ %r)
echo "ENDING run_pretraining.py AT $start_fmt"
