...
#_config_env+=(SEED)
#_config_env+=(PATH LD_LIBRARY_PATH)
_config_env+=(OMPI_DIR SINGULARITY_OMPI_DIR SINGULARITYENV_APPEND_PATH SINGULAIRTYENV_APPEND_LD_LIBRARY_PATH )
_config_env+=(LOCAL_RANK)
...
mlperf_log_utils.mx_resnet_print_event(key=constants.CACHE_CLEAR, val=True)"
        ...
        #mpirun --report-bindings -display-topo -display-map --allow-run-as-root --bind-to none --np ${DGXNGPU} ${SING} ./run_and_time.sh 
        mpirun -display-map --bind-to none --np ${DGXNGPU} ./singularity_run.sh ${SING} # ./run_and_time.sh 
