...
def main(args):
    with jtop() as jetson:
        ...
        if jetson.ok():
            ...
            logging.info("Set jetson_clocks to False")
            ...
            logging.info("Current nvpmodel: %s", jetson.nvpmodel)
            # # You can write a string for a name
            jetson.nvpmodel = args.power_mode
            logging.info("Set nvpmodel to %s", jetson.nvpmodel)
            jetson.fan.speed = 100
            logging.info("Set fan speed to 100%")
        ...
        with open(energy_file_path,"a+") as energy_file:
            ...
            while jetson.ok():
                stats, power = get_jetson_info(jetson)
                # # update process file
...
if __name__ == "__main__":
    ...
    parser.add_argument('--power-mode', 
                        help="Jetson power mode, Available models: ['MAXN', 'MODE_10W', 'MODE_15W', 'MODE_30W_ALL', 'MODE_30W_6CORE', 'MODE_30W_4CORE', 'MODE_30W_2CORE', 'MODE_15W_DESKTOP']",
                        default="MODE_30W_ALL"
                        )
