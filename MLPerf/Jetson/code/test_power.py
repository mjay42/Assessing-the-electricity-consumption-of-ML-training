...
jobname="resnet_power"
nodecount=1
walltime="63:00:00"
node="estats-2"
resources_selection=f"-t exotic -p {node}"
site="toulouse"
reservation="2024-04-19 19:00:00"
...
BASH_LOG="run.log"
EXP_CSV=f"{REPO}/logs/experiment_summary_power.csv"
...
for _ in [1,2]:
    for POWER_MODE in ['MAXN', 'MODE_10W', 'MODE_15W', 'MODE_30W_ALL', 'MODE_30W_6CORE', 'MODE_30W_4CORE', 'MODE_30W_2CORE', 'MODE_15W_DESKTOP']:
        # Initialisation
        date_exp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        LOG_DIR=f"{REPO}/logs/{date_exp}/"
        os.mkdir(LOG_DIR)
        
        open(LOG_DIR+BASH_LOG, 'a').close()
        logging.basicConfig(
            force=True,
        ...
        for k,v in variables.items():
            if not k.startswith("__") and type(v) in [int, float, str]:
                if isinstance(k, str) and k.isupper():
                    k = k.lower()
                to_save[k] = v

        pd.DataFrame([to_save]).to_csv(EXP_CSV, mode='a', header=not os.path.exists(EXP_CSV), index=False)
